...
function Dashboard() {
    ...
    const getTasks = async () => {
        try {
            ...
            else alert(data.error || "Erro ao obter tasks!");
        } catch (error) {
    ...
    const handleLogout = async () => {
        try {
            ...
            else alert(data.error || "Erro ao deslogar!");
        } catch (error) {
            alert("Erro de conexão com o servidor.");
        }
    };
    ...
    const handleAddTask = async (input) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setInput('');
            } else alert(data.error || "Erro ao adicionar task!");
        } catch (error) {
            alert("Erro de conexão com o servidor.");
        }
    };
    ...
    const handleUpdateTask = async (taskId, newDescription, newStatus) => {
        try {
            ...
            else alert(data.error || "Erro ao atualizar task!");
        } catch (error) {
            alert("Erro de conexão com o servidor.");
        }    
    };
    ...
    const handleDeleteTask = async (taskId) => {
        try {
            ...
            else alert(data.error || "Erro ao deletar task!");
        } catch (error) {
            alert("Erro de conexão com o servidor.");
        }    
    };
                    ...
                    <li className={styles['task-item']} key={task._id}>
                        ...
                        <div className={styles['action-buttons']}>
                            <button className={styles['action-btn']} onClick={() => {
                                const newDesc = prompt("Editar tarefa:", task.description);
                                if (newDesc && newDesc.trim() !== "") handleUpdateTask(task._id, newDesc, task.status);
                            }}> 
                                <i className="fa-solid fa-pen"></i>
                            </button>
                                
                            <button className={`${styles['action-btn']} ${styles.delete}`} onClick={() => handleDeleteTask(task._id)}>
